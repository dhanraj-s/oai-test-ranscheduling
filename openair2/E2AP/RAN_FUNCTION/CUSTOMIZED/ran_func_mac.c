...
static void rsrc_alloc(mac_ctrl_msg_t mac_ctrl_msg, module_id_t module_id,
                  frame_t frame,
                  slot_t slot,
                  NR_UE_info_t **UE_list,
                  int max_num_ue,
...
sm_ag_if_ans_t write_ctrl_mac_sm(void const* data)
{
  ...
  int frame = RC.ru[0]->proc.frame_tx;
  int slot = RC.ru[0]->proc.tti_tx;
  // printf("frame: %d\n", frame);
  // printf("slot: %d\n", slot);
  ...
  // printf("Dummy: %d\n", mac_data->hdr.dummy);
  // printf("Action: %d\n", mac_data->msg.action);
  // printf("msg.num_users: %d\n", mac_data->msg.num_users);
  // for(int i=0; i< mac_data->msg.num_users; ++i) {
  //   printf("mcs = %d\n", mac_data->msg.resource_alloc[i].mcs);
  //   printf("num_rb = %d\n", mac_data->msg.resource_alloc[i].num_rb);
  //   printf("user_id (rnti): %d\n", mac_data->msg.resource_alloc[i].user_id);
  //   printf("\n");
  // }
